...
- **claude\_aliases** — the session's claude aliases (the global config's,
  `ATRIUM_CLAUDE_ALIASES` and the fleet's `claude_aliases`), re-installed before
  the first pane spawns so a fleet built on a shim is still claude.
