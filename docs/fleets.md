...
| `memory_mb` | optional | A fixed ceiling, in MiB, on the memory everything the agents run may use (`0` = off). Default: a dynamic ceiling that tracks the machine's free memory. A hard kernel limit on Windows; a soft guard on Linux and macOS. |
