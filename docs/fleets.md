...
| `memory_mb` | optional | A fixed ceiling, in MiB, on the committed memory of everything the agents run (`0` = off). Default: a dynamic ceiling that tracks the machine's free memory. Windows only. |
...
At 90% of the ceiling the guard stops the **largest build process** (a compiler or build script, never an agent) and raises it on the bus and the bar. Past the ceiling, allocations fail inside the panes instead of across the whole machine. The banner's posture line states the guard ("memory guard on"). On unix there is no Job Object; the banner says "no memory guard on this platform".
